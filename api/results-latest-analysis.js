...
export default async function handler(req, res) {
  try {
    ...
    // Traer solo analysis_requested_dt para esa escuela y quedarnos con el MAX en JS (simple y seguro)
    ...
    if (error) {
      console.error("results-latest-analysis supabase error:", error);
      return res.status(500).json({ ok: false, error: error.message || "SUPABASE_READ_FAILED" });
    }

    const max_dt =
      (Array.isArray(data) ? data : [])
        .map(r => r?.analysis_requested_dt)
        .filter(Boolean)
        .sort()
        .slice(-1)[0] || null;

    if (!max_dt) {
      return res.status(200).json({ ok: true, school_id, analysis_dt: null });
    }

    return res.status(200).json({ ok: true, school_id, analysis_dt: max_dt });
  } catch (e) {
    console.error("results-latest-analysis error:", e);
